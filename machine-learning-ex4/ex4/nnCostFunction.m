...
J = ((1/m)*sum(sum(-y_matrix.*log(h_theta) - (1-y_matrix).*log(1-h_theta)))) + ...
				(lambda/(2*m)) * (sum(sum(Theta1_regularised.^2)) + sum(sum(Theta2_regularised.^2)));
...
Theta1_grad = (1/m)*(Delta2'*a1) + (lambda/m)*Theta1_regularised;
Theta2_grad = (1/m)*(Delta3'*a2) + (lambda/m)*Theta2_regularised;
